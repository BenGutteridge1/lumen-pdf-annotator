...
// Build to an isolated staging directory unless an explicit vault plugin path
// is supplied. This prevents development builds from touching any vault by
// accident.
const PLUGIN_DIR =
  process.env.LUMEN_PDF_ANNOTATOR_PLUGIN_DIR ??
...
const WORKER_FILE = require.resolve("pdfjs-dist/build/pdf.worker.min.mjs");
...
function stripPdfJsNodeOnlyAccess(contents, sourcePath) {
  const disabled =
    `new Error("Lumen PDF Annotator disables pdf.js Node-only filesystem/network fallback.")`;
...
function sanitizePdfJsWorker(contents, sourcePath) {
  const unminifiedPostScript = `    if (isEvalSupported && FeatureTest.isEvalSupported) {
      const compiled = new PostScriptCompiler().compile(code, domain, range);
      if (compiled) {
        return new Function("src", "srcOffset", "dest", "destOffset", compiled);
      }
    }`;
  if (contents.includes(unminifiedPostScript)) {
    contents = replaceRequired(
      contents,
      unminifiedPostScript,
      `    if (false) {
      // Runtime code generation is intentionally disabled for Obsidian's CSP.
    }`,
      "worker PostScript eval fast path",
      sourcePath
    );
    contents = sanitizePdfJsApi(contents, sourcePath);
  } else {
    contents = replaceRequired(
      contents,
      `if(t&&FeatureTest.isEvalSupported){const e=(new PostScriptCompiler).compile(g,s,r);if(e)return new Function("src","srcOffset","dest","destOffset",e)}`,
      `if(false){}`,
      "minified worker PostScript eval fast path",
      sourcePath
    );
    contents = replaceRequired(
      contents,
      ...
      "minified worker eval feature test",
      sourcePath
    );
  }
...
function sanitizePdfJsLoadedFile(args) {
  ...
  if (/pdfjs-dist[/\\](legacy[/\\])?image_decoders[/\\]pdf\.image_decoders(\.min)?\.m?js$/.test(sourcePath)) {
...
// we turn this string into a Blob URL module worker. Never a path on disk.
...
const stripPdfJsDynamicScriptFallback = {
  ...
  setup(build) {
    build.onLoad({ filter: /pdfjs-dist[/\\].*\.m?js$/ }, (args) => {
