 ...
 * selection.
 */
import { FileView, TFile, WorkspaceLeaf, Notice, Menu, setIcon } from "obsidian";
...
export const VIEW_TYPE_PDF_ANNOTATOR = "lumen-pdf-annotator-view";
...
const PREFETCH_MARGIN = "720px";
const MAX_CANVAS_PIXELS = 12_000_000;
const ROLL_ROW_HEIGHT = 164;
const ROLL_OVERSCAN = 5;
const MAX_HIGHLIGHT_ALPHA = 0.56;
...
const MARK_STYLE_ICONS: Record<MarkStyle, string> = {
...
export class PdfAnnotatorView extends FileView {
  ...
  private rollDetailEl!: HTMLElement;
  ...
  private inspectorHighlightId: string | null = null;
  private hoverHighlightId: string | null = null;
  private tagPlacementMode = false;
  private rollOpen = false;
  private rollSearchQuery = "";
  private rollFilter: "all" | "highlights" | "notes" = "all";
  private rollFiltered: Highlight[] = [];
  private rollVirtualRaf: number | null = null;
  private storeUnsubscribe: (() => void) | null = null;
  private currentTheme: PdfTheme = "light";
  private searchOpen = false;
  private searchToken = 0;
  private searchTimer: number | null = null;
  private searchResults: PdfSearchResult[] = [];
  private searchMatchesByPage = new Map<number, PdfRect[]>();
  ...
  constructor(
    ...
    private getPdfTheme: () => PdfTheme = () => "light",
    private savePdfTheme: (theme: PdfTheme) => void = () => {}
  ) {
    super(leaf);
  ...
  async onOpen(): Promise<void> {
    initPdfEngine();
    this.currentTheme = this.getPdfTheme();
    this.buildSkeleton();
    ...
    this.registerDomEvent(this.rollListEl, "scroll", () => this.scheduleVirtualRollRender());
    ...
    this.register(() => {
      ...
      this.stopRollScroll();
      if (this.rollVirtualRaf !== null) window.cancelAnimationFrame(this.rollVirtualRaf);
      this.rollVirtualRaf = null;
  ...
  private buildSkeleton(): void {
    ...
    const pageNav = this.toolbarEl.createDiv({ cls: "lpa-page-nav" });
    const previousPage = pageNav.createEl("button", {
      cls: "lpa-tool-button",
      attr: { "aria-label": "Previous page" },
    });
    setIcon(previousPage, "chevron-left");
    previousPage.onclick = () => this.goToPage(this.currentPageIndex() - 1);
    this.pageStatusEl = pageNav.createSpan({ cls: "lpa-page-status", text: "Page —" });
    const nextPage = pageNav.createEl("button", {
      cls: "lpa-tool-button",
      attr: { "aria-label": "Next page" },
    });
    setIcon(nextPage, "chevron-right");
    nextPage.onclick = () => this.goToPage(this.currentPageIndex() + 1);
    const zoomControl = this.toolbarEl.createDiv({ cls: "lpa-zoom-control", attr: { "aria-label": "PDF zoom" } });
    this.zoomOutBtnEl = zoomControl.createEl("button", {
      cls: "lpa-zoom-button",
      attr: { "aria-label": "Zoom out" },
    }) as HTMLButtonElement;
    setIcon(this.zoomOutBtnEl, "minus");
    this.zoomOutBtnEl.onclick = () => this.zoomOut();

    this.zoomLabelEl = zoomControl.createSpan({ cls: "lpa-zoom-label", text: "125%" });
    ...
    this.zoomInBtnEl = zoomControl.createEl("button", {
      cls: "lpa-zoom-button",
      attr: { "aria-label": "Zoom in" },
    }) as HTMLButtonElement;
    setIcon(this.zoomInBtnEl, "plus");
    this.zoomInBtnEl.onclick = () => this.zoomIn();

    const themeControl = this.toolbarEl.createDiv({ cls: "lpa-theme-control", attr: { "aria-label": "PDF theme" } });
    for (const theme of ["light", "sepia", "dark"] as const) {
      const button = themeControl.createEl("button", {
        cls: "lpa-theme-button",
        text: theme[0].toUpperCase() + theme.slice(1),
        attr: { "aria-label": `${theme} PDF theme` },
      });
      button.dataset.theme = theme;
      button.toggleClass("is-active", theme === this.currentTheme);
      button.onclick = () => this.applyTheme(theme, true);
    }
    ...
    this.tagModeBtnEl = this.toolbarEl.createEl("button", {
      cls: "lpa-toolbar-icon-button",
      attr: { "aria-label": "Place a page note tag" },
    }) as HTMLButtonElement;
    setIcon(this.tagModeBtnEl, "sticky-note");
    ...
    setIcon(this.annotationsBtnEl, "list");
    this.annotationsBtnEl.onclick = () => this.setRollOpen(!this.rollOpen);

    this.searchButtonEl = this.toolbarEl.createEl("button", {
      cls: "lpa-toolbar-icon-button",
      attr: { "aria-label": "Search PDF" },
    }) as HTMLButtonElement;
    setIcon(this.searchButtonEl, "search");
    this.searchButtonEl.onclick = () => this.setSearchOpen(!this.searchOpen);

    this.searchPanelEl = this.rootEl.createDiv({ cls: "lpa-pdf-search is-hidden" });
    const searchHead = this.searchPanelEl.createDiv({ cls: "lpa-pdf-search-head" });
    this.searchInputEl = searchHead.createEl("input", {
      attr: { type: "search", placeholder: "Search this PDF", "aria-label": "Search this PDF" },
    });
    this.searchInputEl.oninput = () => {
      if (this.searchTimer !== null) window.clearTimeout(this.searchTimer);
      this.searchTimer = window.setTimeout(() => {
        this.searchTimer = null;
        void this.runPdfSearch(this.searchInputEl.value);
      }, 180);
    };
    const closeSearch = searchHead.createEl("button", { text: "×", attr: { "aria-label": "Close PDF search" } });
    setIcon(closeSearch, "x");
    closeSearch.onclick = () => this.setSearchOpen(false);
    this.searchStatusEl = this.searchPanelEl.createDiv({ cls: "lpa-pdf-search-status", text: "Type to search every page" });
    this.searchResultsEl = this.searchPanelEl.createDiv({ cls: "lpa-pdf-search-results" });

    this.rollOpen = false;
    this.rootEl.addClass("is-inspector-closed");
    ...
    const filterRow = this.rollEl.createDiv({ cls: "lpa-roll-filters", attr: { role: "tablist" } });
    for (const filter of ["all", "highlights", "notes"] as const) {
      const button = filterRow.createEl("button", {
        cls: "lpa-roll-filter",
        text: filter[0].toUpperCase() + filter.slice(1),
        attr: { role: "tab", "aria-selected": filter === "all" ? "true" : "false" },
      });
      button.dataset.filter = filter;
      button.toggleClass("is-active", filter === "all");
      button.onclick = () => {
        this.rollFilter = filter;
        for (const candidate of Array.from(filterRow.children) as HTMLElement[]) {
          const active = candidate.dataset.filter === filter;
    ...
    this.rollDetailEl = this.rollEl.createDiv({ cls: "lpa-roll-detail is-hidden" });
    this.rollListEl = this.rollEl.createDiv({ cls: "lpa-roll-list" });

    this.bodyEl = this.rootEl.createDiv({ cls: "lpa-body" });
    this.leftMarginEl = this.bodyEl.createDiv({ cls: "lpa-margin lpa-margin-left is-retired", attr: { "aria-hidden": "true" } });
    this.pagesEl = this.bodyEl.createDiv({ cls: "lpa-pages" });
    this.rightMarginEl = this.bodyEl.createDiv({ cls: "lpa-margin lpa-margin-right is-retired", attr: { "aria-hidden": "true" } });
    ...
    this.leftMarginToggleEl.addClass("is-retired");
    this.rightMarginToggleEl.addClass("is-retired");
  ...
  /** Public command surface used by Obsidian commands and global hotkeys. */
  togglePdfSearch(): boolean {
    this.setSearchOpen(!this.searchOpen);
    return this.searchOpen;
  }

  toggleAnnotationInspector(): boolean {
    this.setRollOpen(!this.rollOpen);
    return this.rollOpen;
  }

  togglePageNotePlacement(): boolean {
    this.setTagPlacementMode(!this.tagPlacementMode);
    return this.tagPlacementMode;
  }
  ...
  private setRollOpen(on: boolean): void {
    ...
    this.rootEl?.toggleClass("is-inspector-closed", !on);
  ...
  private setSearchOpen(on: boolean): void {
    this.searchOpen = on;
    this.searchPanelEl?.toggleClass("is-hidden", !on);
    this.searchButtonEl?.toggleClass("is-active", on);
    this.searchButtonEl?.setAttribute("aria-pressed", on ? "true" : "false");
    if (on) {
      window.setTimeout(() => {
      ...
      this.searchResults = [];
      this.searchMatchesByPage.clear();
      this.searchTotalMatches = 0;
      this.searchStatusEl?.setText("Type to search every page");
      this.renderSearchResults();
      // Search hits share the lightweight note overlay. Repainting only pages
      // that are already rendered removes every visible hit immediately while
      // keeping the page virtualization boundary intact.
      for (const pv of this.pageViews) if (pv.rendered) this.renderTags(pv);
    }
  }

  /** Floating reader UI is intentionally session-transient. A newly loaded
   * document always starts with an unobstructed page, even if the previous PDF
   * had an inspector detail or search query open. */
  private resetTransientPanels(): void {
    this.inspectorHighlightId = null;
    this.rollSearchQuery = "";
    this.rollFilter = "all";
    if (this.rollSearchEl) this.rollSearchEl.value = "";
    for (const button of Array.from(this.rollEl?.querySelectorAll<HTMLElement>(".lpa-roll-filter") ?? [])) {
      const active = button.dataset.filter === "all";
      button.toggleClass("is-active", active);
      button.setAttribute("aria-selected", active ? "true" : "false");
    }
    this.rollDetailEl?.empty();
    this.rollDetailEl?.addClass("is-hidden");
    this.rollEl?.removeClass("has-detail");
    this.setRollOpen(false);
    this.setSearchOpen(false);
  }

  private async runPdfSearch(rawQuery: string): Promise<void> {
    ...
    const totalPages = this.pdfDoc.numPages as number;
    this.searchStatusEl?.setText(`Searching 0 of ${totalPages} pages…`);
    for (let pageIndex = 0; pageIndex < totalPages; pageIndex++) {
      if (token !== this.searchToken) return;
      try {
        const page = this.pageViews[pageIndex]?.page ?? await this.pdfDoc.getPage(pageIndex + 1);
        const content = await page.getTextContent();
        const pageRects: PdfRect[] = [];
        const items = content.items as any[];
        // Cache normalized strings once per page. Richer excerpts then remain a
        // bounded slice around each hit instead of adding another page scan.
        const itemTexts = items.map((item) => String(item.str ?? "").replace(/\s+/g, " ").trim());
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
          if (token !== this.searchToken) return;
          const item = items[itemIndex];
          const text = itemTexts[itemIndex];
          const normalized = text.toLowerCase();
          let at = normalized.indexOf(query);
          while (at >= 0) {
            this.searchTotalMatches++;
            const width = Number(item.width) || Math.max(8, text.length * 5);
            const height = Math.max(6, Number(item.height) || Math.abs(Number(item.transform?.[3])) || 10);
            const x = Number(item.transform?.[4]) || 0;
            const baseline = Number(item.transform?.[5]) || 0;
            const start = text.length ? at / text.length : 0;
            const end = text.length ? (at + query.length) / text.length : 1;
            const rect: PdfRect = {
              x1: x + width * start,
              y1: baseline - height * 0.2,
              x2: x + width * Math.min(1, end),
              y2: baseline + height * 0.8,
            };
            pageRects.push(rect);
            if (this.searchResults.length < 2_000) {
              const left = Math.max(0, at - 92);
              const right = Math.min(text.length, at + query.length + 132);
              const contextStart = Math.max(0, itemIndex - 3);
              const contextEnd = Math.min(itemTexts.length, itemIndex + 4);
              this.searchResults.push({
                page: pageIndex,
                snippetBefore: `${left ? "…" : ""}${text.slice(left, at)}`,
                match: text.slice(at, at + query.length),
                snippetAfter: `${text.slice(at + query.length, right)}${right < text.length ? "…" : ""}`,
                context: shortAnnotationText(
                  itemTexts
                    .slice(contextStart, contextEnd)
                    .filter((value, offset) => Boolean(value) && contextStart + offset !== itemIndex)
                    .join(" "),
                  360
                ),
                rects: [rect],
            ...
            at = normalized.indexOf(query, at + Math.max(1, query.length));
          }
        }
        if (pageRects.length) {
          this.searchMatchesByPage.set(pageIndex, pageRects);
          const pv = this.pageViews[pageIndex];
          if (pv?.rendered) this.renderTags(pv);
        }
      } catch (error) {
        console.warn(`${LOG_TAG} search skipped page ${pageIndex + 1}`, error);
      }
      if (pageIndex % 6 === 5 || pageIndex === totalPages - 1) {
        this.searchStatusEl?.setText(
          `Searching ${pageIndex + 1} of ${totalPages} pages · ${this.searchTotalMatches} found`
        );
        this.renderSearchResults();
    ...
    if (token !== this.searchToken) return;
    this.searchStatusEl?.setText(
      `${this.searchTotalMatches} ${this.searchTotalMatches === 1 ? "match" : "matches"} in ${totalPages} pages`
    );
    this.renderSearchResults();
  }
  ...
  private renderSearchResults(): void {
    ...
    const visible = this.searchResults.slice(0, 160);
    for (const result of visible) {
      const button = this.searchResultsEl.createEl("button", { cls: "lpa-pdf-search-result" });
      const head = button.createSpan({ cls: "lpa-pdf-search-result-head" });
      head.createSpan({ cls: "lpa-pdf-search-page", text: `p.${result.page + 1}` });
      head.createSpan({ cls: "lpa-pdf-search-kind", text: "PDF match" });
      const snippet = button.createSpan({ cls: "lpa-pdf-search-snippet" });
      snippet.appendChild(snippet.ownerDocument.createTextNode(result.snippetBefore));
      snippet.createEl("mark", { cls: "lpa-pdf-search-term", text: result.match || "Match" });
      snippet.appendChild(snippet.ownerDocument.createTextNode(result.snippetAfter));
      if (result.context) button.createSpan({ cls: "lpa-pdf-search-context", text: result.context });
      button.onclick = () => void this.revealSearchResult(result);
    }
    if (this.searchResults.length > visible.length) {
      this.searchResultsEl.createDiv({
        cls: "lpa-pdf-search-more",
        text: `${this.searchResults.length - visible.length} more results — refine the search to narrow them`,
      });
    }
  }

  private async revealSearchResult(result: PdfSearchResult): Promise<void> {
    this.goToPage(result.page);
    const pv = this.pageViews[result.page];
    if (!pv) return;
    await this.renderPageContent(pv);
    this.renderTags(pv);
    window.setTimeout(() => {
      for (const hit of Array.from(pv.noteLayer.querySelectorAll<HTMLElement>(".lpa-search-hit"))) {
        hit.addClass("is-flash");
      }
    }, 0);
  }

  private renderSearchMatches(pv: PageView): void {
    if (!pv.page) return;
    const rects = this.searchMatchesByPage.get(pv.index) ?? [];
    const viewport = pv.page.getViewport({ scale: this.scale });
    for (const rect of this.rectToViewport(viewport, rects)) {
      const hit = pv.noteLayer.createDiv({ cls: "lpa-search-hit" });
      hit.setCssProps({
        left: `${rect.left}px`,
        top: `${rect.top}px`,
        width: `${Math.max(2, rect.right - rect.left)}px`,
        height: `${Math.max(4, rect.bottom - rect.top)}px`,
      });
    }
  }

  private applyTheme(theme: PdfTheme, persist: boolean): void {
    this.currentTheme = theme;
    if (this.rootEl) {
      this.rootEl.dataset.pdfTheme = theme;
      for (const button of Array.from(this.rootEl.querySelectorAll<HTMLElement>(".lpa-theme-button"))) {
        button.toggleClass("is-active", button.dataset.theme === theme);
      }
    }
    if (persist) this.savePdfTheme(theme);
  }

  private currentPageIndex(): number {
    if (!this.pagesEl) return 0;
    const viewport = this.pagesEl.getBoundingClientRect();
    let best = { index: 0, area: -1 };
    const candidates = this.visible.size ? this.visible : new Set(this.pageViews.map((_, index) => index));
    for (const index of candidates) {
      const rect = this.pageViews[index]?.el.getBoundingClientRect();
      if (!rect) continue;
  ...
  private goToPage(index: number): void {
    ...
    const target = clamp(0, Math.round(index), this.pageViews.length - 1);
    this.pageViews[target]?.el.scrollIntoView({ block: "start" });
    this.updatePageStatus(target);
  }

  private updatePageStatus(index = this.currentPageIndex()): void {
    if (!this.pageStatusEl) return;
    const total = this.pdfDoc?.numPages ?? 0;
    this.pageStatusEl.setText(total ? `Page ${index + 1} of ${total}` : "Page —");
  }
  ...
  private shouldPreserveActiveSelection(evt: PointerEvent): boolean {
    ...
    if (target.closest(".lpa-page-tag, .lpa-margin-card, .lpa-roll-item, .lpa-roll-detail, .lpa-mark-popover, .lpa-selection-popover, .lpa-rubber-handle")) return true;
  ...
  async onLoadFile(file: TFile): Promise<void> {
    ...
    this.storeUnsubscribe = this.store.subscribe((change) => {
      this.annotationCountEl?.setText(
        `${this.store?.size ?? 0} ${(this.store?.size ?? 0) === 1 ? "annotation" : "annotations"}`
      );
      if (change.type === "reset" || change.page === this.currentPageIndex()) {
        const pv = change.type === "reset" ? null : this.pageViews[change.page];
        if (pv?.rendered) {
      ...
      if (change.type !== "update") {
        this.renderAnnotationRollList();
        this.renderInspectorDetail();
      }
    });

    await this.buildPages();
    this.renderAnnotationSidebar();
    this.updatePageStatus(0);
  }
  ...
  private renderAnnotationSidebar(): void {
    if (!this.annotationCountEl) return;
    const annotations = [...(this.store?.allSorted() ?? [])];
    ...
    if (this.inspectorHighlightId && !ids.has(this.inspectorHighlightId)) this.inspectorHighlightId = null;
  ...
  private renderAnnotationRollList(): void {
    ...
    this.rollFiltered = annotations.filter((h) => {
      if (this.rollFilter === "highlights" && annotationTypeOf(h) !== "highlight") return false;
      if (this.rollFilter === "notes" && !(h.note?.trim() || h.noteContentCJK?.trim())) return false;
      return !query || annotationMatchesSearch(h, query);
    });

    this.rollMetaEl.setText(
      query
        ? `${this.rollFiltered.length}/${annotations.length}`
    ...
    if (this.rollFiltered.length === 0) {
      this.rollListEl.empty();
      this.rollListEl.createDiv({ cls: "lpa-roll-empty", text: "No matching annotations." });
      return;
    }
    this.renderVirtualRollWindow();
  }

  private scheduleVirtualRollRender(): void {
    if (this.rollVirtualRaf !== null) return;
    this.rollVirtualRaf = window.requestAnimationFrame(() => {
      this.rollVirtualRaf = null;
      this.renderVirtualRollWindow();
    });
  }

  private renderVirtualRollWindow(): void {
    if (!this.rollListEl || !this.rollFiltered.length) return;
    const scrollTop = this.rollListEl.scrollTop;
    const viewportHeight = this.rollListEl.clientHeight || 500;
    const start = Math.max(0, Math.floor(scrollTop / ROLL_ROW_HEIGHT) - ROLL_OVERSCAN);
    const end = Math.min(
      this.rollFiltered.length,
      Math.ceil((scrollTop + viewportHeight) / ROLL_ROW_HEIGHT) + ROLL_OVERSCAN
    );
    this.rollListEl.empty();
    const spacer = this.rollListEl.createDiv({ cls: "lpa-roll-spacer" });
    spacer.setCssProps({ height: `${this.rollFiltered.length * ROLL_ROW_HEIGHT}px` });
    for (let i = start; i < end; i++) this.createRollItem(spacer, this.rollFiltered[i], i);
  }

  private createRollItem(parent: HTMLElement, h: Highlight, index: number): HTMLElement {
    const type = annotationTypeOf(h);
    const accent = brightAnnotationColor(annotationColor(h));
    const item = parent.createDiv({ cls: `lpa-roll-item lpa-roll-item--${type}` });
    item.setCssProps({ top: `${index * ROLL_ROW_HEIGHT}px`, height: `${ROLL_ROW_HEIGHT - 8}px` });
    item.dataset.hlId = h.id;
    item.dataset.annotationId = h.id;
    item.style.setProperty("--lpa-accent", accent);
    item.style.setProperty("--lpa-card-tint", stickerBackgroundColor(annotationColor(h), 0.12));
  ...
  private renderInspectorDetail(): void {
    if (!this.rollDetailEl) return;
    const h = this.inspectorHighlightId ? this.store?.get(this.inspectorHighlightId) : null;
    this.rollDetailEl.empty();
    this.rollDetailEl.toggleClass("is-hidden", !h);
    this.rollEl?.toggleClass("has-detail", !!h);
    if (!h) return;
    const head = this.rollDetailEl.createDiv({ cls: "lpa-detail-head" });
    head.createSpan({ cls: "lpa-detail-page", text: `Page ${h.page + 1}` });
    head.createSpan({ cls: "lpa-detail-kind", text: annotationKindLabel(h) });
    const close = head.createEl("button", { attr: { "aria-label": "Close annotation editor" } });
    setIcon(close, "x");
    close.onclick = () => {
      this.inspectorHighlightId = null;
      this.renderInspectorDetail();
    ...
    if (h.text) {
      const source = this.rollDetailEl.createDiv({
        cls: "lpa-detail-source",
        text: h.text,
        attr: {
          tabindex: "0",
          role: "button",
          "aria-label": "Full highlighted PDF text",
          "aria-expanded": "false",
        },
      });
      const toggleSource = () => {
        const expanded = !source.hasClass("is-expanded");
        source.toggleClass("is-expanded", expanded);
        source.setAttribute("aria-expanded", expanded ? "true" : "false");
      };
      source.onclick = toggleSource;
      source.onkeydown = (evt) => {
    ...
    const controls = this.rollDetailEl.createDiv({ cls: "lpa-detail-controls" });
    const colorGroup = controls.createDiv({ cls: "lpa-detail-control-group" });
    colorGroup.createDiv({ cls: "lpa-detail-control-label", text: "Colour" });
    const colors = colorGroup.createDiv({ cls: "lpa-detail-colors", attr: { "aria-label": "Annotation colour" } });
    for (const palette of PALETTE) {
      const button = colors.createEl("button", { attr: { "aria-label": palette.name } });
      button.style.setProperty("--lpa-swatch", palette.fill);
      button.toggleClass("is-active", resolvePalette(h.color)?.name === palette.name);
      button.onclick = () => {
        this.store?.update(h.id, { color: palette.fill, tagColor: annotationTypeOf(h) === "tag" ? palette.fill : h.tagColor });
        this.renderInspectorDetail();
      };
    }

    if (annotationTypeOf(h) === "highlight") {
      const styleGroup = controls.createDiv({ cls: "lpa-detail-control-group" });
      styleGroup.createDiv({ cls: "lpa-detail-control-label", text: "Style" });
      const styles = styleGroup.createDiv({ cls: "lpa-detail-styles", attr: { "aria-label": "Annotation style" } });
      for (const style of MARK_STYLES) {
        const button = styles.createEl("button", {
          attr: { "aria-label": MARK_STYLE_LABELS[style] },
        });
        buildStylePreview(button, style);
        button.toggleClass("is-active", markStyleOf(h) === style);
        button.onclick = () => {
          this.store?.update(h.id, { style });
          this.renderInspectorDetail();
        };
      }
    }

    const note = this.rollDetailEl.createEl("textarea", {
      cls: "lpa-detail-note",
      attr: { rows: "5", placeholder: "Add a note…", "aria-label": "Annotation note" },
    });
    note.value = h.note ?? "";
    note.oninput = () => this.store?.update(h.id, { note: note.value });
    note.onblur = () => this.renderAnnotationRollList();
    const tags = this.rollDetailEl.createEl("input", {
      cls: "lpa-detail-tags",
      attr: { type: "text", placeholder: "Add tags…", "aria-label": "Annotation tags" },
    });
    tags.value = (h.tags ?? []).join(", ");
    tags.onchange = () => {
      this.store?.update(h.id, {
        tags: tags.value.split(",").map((tag) => tag.trim()).filter(Boolean),
      });
      this.renderAnnotationRollList();
    };
    const actions = this.rollDetailEl.createDiv({ cls: "lpa-detail-actions" });
  ...
  private activateHighlight(
    id: string | null,
    options: { scrollSidebar?: boolean; focusNote?: boolean; openInspector?: boolean } = {}
  ): void {
    this.activeHighlightId = id;
    if (id && !this.store?.get(id)) this.activeHighlightId = null;
    if (!this.activeHighlightId) this.inspectorHighlightId = null;
    const openInspector = options.openInspector ?? !!(options.scrollSidebar || options.focusNote);
    if (this.activeHighlightId && openInspector) {
      this.inspectorHighlightId = this.activeHighlightId;
      this.setRollOpen(true);
    }
    this.renderInspectorDetail();
    this.syncHighlightBindingState();
    if (this.activeHighlightId && this.rollOpen) this.scrollRollItem(this.activeHighlightId);
    if (this.activeHighlightId && options.focusNote) {
      window.setTimeout(() => this.rollDetailEl?.querySelector<HTMLTextAreaElement>(".lpa-detail-note")?.focus(), 0);
    }
  }
  ...
  private scrollRollItem(id: string): void {
    window.setTimeout(() => {
      ...
      const index = this.rollFiltered.findIndex((item) => item.id === id);
      if (index < 0) return;
      const top = index * ROLL_ROW_HEIGHT;
      const bottom = top + ROLL_ROW_HEIGHT;
      if (top < this.rollListEl.scrollTop) this.rollListEl.scrollTop = top;
      else if (bottom > this.rollListEl.scrollTop + this.rollListEl.clientHeight) {
        this.rollListEl.scrollTop = Math.max(0, bottom - this.rollListEl.clientHeight);
      }
      this.renderVirtualRollWindow();
    }, 0);
  }
  ...
  private async renderPageContent(pv: PageView): Promise<void> {
    ...
    try {
      ...
      const pixelArea = Math.max(1, viewport.width * viewport.height);
      const dpr = Math.min(desiredDpr, Math.sqrt(MAX_CANVAS_PIXELS / pixelArea));
  ...
  private computeSelectionActionAnchor(sel: Selection, rects: DOMRect[]): SelectionActionAnchor | null {
    ...
    // The accepted canvas design centers the command surface underneath the
    // passage, rather than attaching a tiny cluster to the focus caret.
    return {
      x: left + (right - left) / 2,
      y: top,
      height: bottom - top,
  ...
  private openSelectionActionPopup(): void {
    ...
    pop.dataset.pdfTheme = this.currentTheme;
    ...
    const moreBtn = pop.createEl("button", {
      cls: "lpa-selection-action lpa-selection-more",
      attr: { "aria-label": "Choose highlight style", "aria-expanded": "false" },
    });
    setIcon(moreBtn, "ellipsis");

    pop.createDiv({ cls: "lpa-selection-divider", attr: { "aria-hidden": "true" } });

    const annotateBtn = pop.createEl("button", {
      cls: "lpa-selection-action",
      attr: { "aria-label": "Annotate selection" },
    });
    setIcon(annotateBtn, "message-square");
    ...
    const copyBtn = pop.createEl("button", {
      cls: "lpa-selection-action lpa-selection-action-quiet",
      attr: { "aria-label": "Copy selection" },
    });
    ...
    const dismissBtn = pop.createEl("button", {
      cls: "lpa-selection-action lpa-selection-dismiss",
      attr: { "aria-label": "Dismiss selection actions" },
    });
    ...
    const styleTray = pop.createDiv({
      cls: "lpa-selection-style-tray is-hidden",
      attr: { role: "radiogroup", "aria-label": "Highlight style" },
    });
    for (const style of MARK_STYLES) {
      const button = styleTray.createEl("button", {
        cls: "lpa-selection-style",
        attr: { "aria-label": MARK_STYLE_LABELS[style] },
      });
      button.dataset.style = style;
      buildStylePreview(button, style);
      button.toggleClass("is-active", this.currentStyle === style);
      button.onclick = (evt) => {
        evt.preventDefault();
        this.setActiveStyle(style);
        for (const candidate of Array.from(styleTray.children) as HTMLElement[]) {
          candidate.toggleClass("is-active", candidate.dataset.style === style);
        }
      };
    }
    moreBtn.onclick = (evt) => {
      evt.preventDefault();
      const expanded = styleTray.hasClass("is-hidden");
      styleTray.toggleClass("is-hidden", !expanded);
      pop.toggleClass("is-expanded", expanded);
      moreBtn.setAttribute("aria-expanded", expanded ? "true" : "false");
      this.positionSelectionPopover(pop, pending, doc);
    };
  ...
  private positionSelectionPopover(pop: HTMLElement, pending: PendingSelection, doc: Document): void {
    ...
    let x: number;
    let y: number;
    if (pending.anchor.side === "center") {
      x = pending.anchor.x - pr.width / 2;
      y = pending.anchor.y + pending.anchor.height + 9;
      if (y + pr.height > vh - 8) y = pending.anchor.y - pr.height - 9;
    } else {
      const sideGap = 10;
      x = pending.anchor.side === "right"
        ? pending.anchor.x + sideGap
        : pending.anchor.x - pr.width - sideGap;
      y = pending.anchor.y + pending.anchor.height / 2 - pr.height / 2;
    }
  ...
  private commitPendingSelection(mode: "highlight" | "annotate"): void {
    ...
    if (mode === "annotate") {
      const created = this.store.get(createdIds[0]);
      const pv = created ? this.pageViews[created.page] : null;
      this.activateHighlight(createdIds[0], { openInspector: false });
      if (created && pv) {
        this.openMarkPopover(
          created,
          { clientX: pending.anchor.x, clientY: pending.anchor.y + pending.anchor.height },
          pv,
          { focusNote: true }
        );
      }
  ...
  private handleHighlightClick(evt: MouseEvent): void {
    ...
    if (hit) {
      evt.preventDefault();
      // The anchored editor is independent from the inspector. Clicking a PDF
      // mark may visually activate it, but never opens a second editor in the
      // right rail unless the user explicitly chooses "Open in inspector".
      this.inspectorHighlightId = null;
      this.activateHighlight(hit.highlight.id, { openInspector: false });
  ...
  private openMarkPopover(
    ...
    pop.dataset.pdfTheme = this.currentTheme;
    const popHead = pop.createDiv({ cls: "lpa-mark-popover-head" });
    popHead.createSpan({ text: `Page ${h.page + 1}` });
    popHead.createSpan({ cls: "lpa-mark-popover-kind", text: annotationKindLabel(h) });
    const closeEditor = popHead.createEl("button", {
      cls: "lpa-mark-popover-close",
      attr: { "aria-label": "Close individual editor" },
    });
    setIcon(closeEditor, "x");
    closeEditor.onclick = () => this.closeMarkPopover();
    if (h.text) pop.createDiv({ cls: "lpa-mark-popover-source", text: h.text });
    ...
    const controlShelf = pop.createDiv({ cls: "lpa-editor-controls" });
    const colorGroup = controlShelf.createDiv({ cls: "lpa-editor-control-group" });
    colorGroup.createDiv({ cls: "lpa-editor-control-label", text: "Colour" });
    const colorRow = colorGroup.createDiv({ cls: "lpa-swatches", attr: { "aria-label": "Highlight colour" } });
    const styleGroup = controlShelf.createDiv({ cls: "lpa-editor-control-group" });
    styleGroup.createDiv({ cls: "lpa-editor-control-label", text: "Style" });
    const styleRow = styleGroup.createDiv({ cls: "lpa-styles", attr: { role: "radiogroup", "aria-label": "Mark style" } });
    ...
    for (const st of MARK_STYLES) {
      const btn = styleRow.createEl("button", {
        cls: "lpa-style-btn",
        attr: { "aria-label": MARK_STYLE_LABELS[st] },
      });
    ...
    const noteEditor = pop.createEl("textarea", {
      cls: "lpa-mark-popover-note",
      attr: { rows: "3", placeholder: "Add a note…", "aria-label": "Annotation note" },
    });
    noteEditor.value = this.store?.get(h.id)?.note ?? "";
    noteEditor.oninput = () => this.store?.update(h.id, { note: noteEditor.value });
    noteEditor.onblur = () => this.renderAnnotationRollList();

    const tagEditor = pop.createEl("input", {
      cls: "lpa-mark-popover-tags",
      attr: { type: "text", placeholder: "Add tags…", "aria-label": "Annotation tags" },
    });
    tagEditor.value = (this.store?.get(h.id)?.tags ?? []).join(", ");
    tagEditor.onchange = () => {
      const tags = tagEditor.value.split(",").map((tag) => tag.trim()).filter(Boolean);
      this.store?.update(h.id, { tags });
      this.renderAnnotationRollList();
    };
    ...
    const copyBtn = actions.createEl("button", {
      attr: { "aria-label": "Copy annotation text" },
    });
    setIcon(copyBtn, "copy");
    ...
    const noteBtn = actions.createEl("button", {
      attr: { "aria-label": "Open annotation in inspector" },
    });
    setIcon(noteBtn, "panel-right");
    noteBtn.onclick = () => {
    ...
    delBtn.onclick = () => {
      ...
      if (this.inspectorHighlightId === h.id) this.inspectorHighlightId = null;
    ...
    if (options.focusNote) window.setTimeout(() => noteEditor.focus(), 0);
  }
  ...
  private deleteAnnotation(id: string): void {
    ...
    if (this.inspectorHighlightId === id) this.inspectorHighlightId = null;
  ...
  private scheduleMarginLayout(): void {
    // The v0.4 inspector is fixed outside the page flow. Keeping this method as
    // a no-op preserves call-site compatibility while deliberately avoiding the
    // old scroll/resize card measurement pipeline.
  }
  ...
  async importLegacyAnnotations(): Promise<void> {
    ...
    const seen = new Set(
      this.store.allSorted().map((h) => `${h.page}|${dedupeKey(h.text)}`)
    );
  ...
  private onDocumentKeyDown(evt: KeyboardEvent): void {
    ...
    if (evt.key === "Escape" && this.searchOpen) {
      evt.preventDefault();
      this.setSearchOpen(false);
      return;
    }
...
/** Opaque palette hue for thin UI edges. Using the darker drawing ink here
 * made colour identifiers recede too far on dark PDF themes. */
function brightAnnotationColor(color: string): string {
  const pal = resolvePalette(color);
  const c = parseColor(pal?.fill ?? color);
  return c ? `rgb(${c.r}, ${c.g}, ${c.b})` : pal?.fill ?? color;
}
...
/** Build a compact, accessible style glyph without text-heavy controls. */
function buildStylePreview(btn: HTMLElement, style: MarkStyle): void {
  btn.empty();
  setIcon(btn, MARK_STYLE_ICONS[style]);
  btn.addClass(`lpa-style-glyph--${style}`);
}
