...
export class NativePdfOverlay {
  ...
  ensureToolbarButtons(group: HTMLElement): void {
    ...
    this.tagBtn = group.createEl("button", {
      ...
      attr: { type: "button", "aria-label": "Place a page note tag" },
    });
    ...
    this.listBtn = group.createEl("button", {
      cls: "lpa-native-btn clickable-icon",
      attr: { type: "button", "aria-label": "Show annotations" },
    });
  ...
  private showSelectionPopover(x: number, y: number): void {
    ...
    for (const style of MARK_STYLES) {
      ...
      button.createSpan({ cls: `lpa-style-sample lpa-style-sample--${style}`, text: "A", attr: { "aria-hidden": "true" } });
    ...
    moreBtn.onclick = (evt) => {
      ...
      const next = pop.getBoundingClientRect();
      pop.setCssProps({
        left: `${clamp(8, x - next.width / 2, Math.max(8, vw - next.width - 8))}px`,
        top: `${clamp(8, y + 14, Math.max(8, vh - next.height - 8))}px`,
      });
    };
  ...
  private syncCardContent(card: HTMLElement, h: Highlight): void {
    card.style.setProperty("--lpa-accent", brightAnnotationColor(annotationColor(h)));
  ...
  private renderListItems(): void {
    ...
    for (const h of filtered) {
      ...
      item.style.setProperty(
        "--lpa-accent",
        brightAnnotationColor(annotationColor(h))
      );
